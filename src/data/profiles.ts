...
export const PROFILES: Record<ProfileId, ProfileConfig> = {
  ben: {
    ...
    knownGoodLooks: [
      ...
      'Taylor & Stitch lighter tan work jacket open over a white tee, KOTN brown corduroy trouser, Mephisto Rainbow moccasin hiker — the light jacket over the one dark garment, the trouser, with the shoe picking the trouser back up',
      'Mac Weldon shirt open over a white tee, Toast brick/terracotta wide-leg trouser with a single cuff, Diadora Equipe Vela SW, brown belt — no dark piece anywhere; two mid tones held apart by the white tee and the cream sneaker',
      '45R indigo linen short-sleeve overshirt open over a white tee, G1 Goods sage/olive utility trouser cuffed once, Mephisto Rainbow moccasin hiker, brown belt — the darkest piece on top for once rather than at the bottom, over a mid trouser, with the white tee showing through the open front',
      ...
      'Berner Kühl blue-grey zip knit zipped shut over a white tee, Carhartt Parrish Pant in obsidian stone grey, Jacques Solovière near-black pebbled hiker, Shiny black crossbody — the one closed layer. Only the tee hem shows below the knit, so nothing light runs between them; the grey knit reading lighter than the obsidian trouser is what separates the two',
      'mfpen charcoal wool zip jacket (cropped) over a cream tee, mfpen stone/greige flannel trouser, Jacques Solovière near-black pebbled hiker — a cropped jacket against a high full trouser; the jacket is the one dark garment, the light trouser carries the contrast, and the near-black shoe is exempt',
    ],
  },
  yona: {
    id: 'yona',
