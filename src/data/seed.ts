...
function item(
  ...
  name: string,
  color: string,
  extra?: Partial<Pick<Item, 'brand' | 'rise' | 'functional'>>,
): Item {
  counter += 1
  return {
    id: `seed-${counter}`,
...
function benCloset(): Item[] {
  return [
    ...
    item('Shirt', 'Ticking stripe shirt', 'cream/navy'),
    ...
    item('Basics/Tee', 'KOTN tee', 'white'),
    ...
    // Trousers (rise in inches — anything under 13.5" gets flagged in the Closet view)
    ...
    item('Footwear', 'Danner tan suede hiker', 'tan'),
...
function benGaps(): Gap[] {
  return [
    ...
    gap(
      'Vintage Baracuta G9 Harrington',
      'medium',
      'Natural/stone or sunfaded, size 44 / US Medium',
    ),
